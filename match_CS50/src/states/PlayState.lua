...
function PlayState:matchcheck()
    for i = 1,8 do
        for j = 1,8 do
            if i-1>=1 then
                local t1 = self.board.tiles[i][j]
                local t2 = self.board.tiles[i-1][j]
            ...
            if j-1 >= 1 then
                local t1 = self.board.tiles[i][j]
                local t2 = self.board.tiles[i][j-1]
    ...
    if self.canInput then
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
                ...
                t1 = self.highlightedTile
                t2 = self.board.tiles[y][x]
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
            ...
            for k,tile in pairs(match) do
                multiplier = multiplier + tile.variety
            end
            self.score = self.score + multiplier * 10 * #match
